...
import { useEarningsBreakdown } from '@/hooks/useEarningsBreakdown';

const EarningsAnalyticsPage = () => {
  const [startDate, setStartDate] = useState('2025-06-01');
  const [endDate, setEndDate] = useState('2025-06-14');
  const { data, loading } = useEarningsBreakdown(startDate, endDate);
  ...
  return (
    <div className="p-6 space-y-6 dark:bg-black dark:text-white min-h-screen">
      ...
      <EarningsBreakdownChart data={data} loading={loading} />
    </div>
  );
};
